...
public abstract class MappedPerformanceTracker {
    /*
     * This class provides a way to track performance metrics such as first
     * paint, instant fps, average fps.<p>
     *
     * Typical use scenario is to obtain the tracker from a {@code MappedScene}, and use it
    ...
    public static boolean isLoggingEnabled() {
        return MappedToolkit.getToolkit().getPerformanceTracker().perfLoggingEnabled;
    }

    public static abstract class SceneAccessor {
        public abstract void setPerfTracker(MappedScene scene, MappedPerformanceTracker tracker);
        public abstract MappedPerformanceTracker getPerfTracker(MappedScene scene);
    }

    /*
     * Creates a {@code MappedPerformanceTracker} for this scene. There could be only one
     ...
     * is released with {@link #releaseSceneTracker(MappedScene)}. <p>
     *
     * @return an instance of {@code MappedPerformanceTracker} associated with the scene
     * or null if the tracker couldn't be created.
     */
    public static MappedPerformanceTracker getSceneTracker(MappedScene scene) {
        MappedPerformanceTracker tracker = null;
        if (sceneAccessor != null) {
            tracker = sceneAccessor.getPerfTracker(scene);
            if (tracker == null) {
                tracker = MappedToolkit.getToolkit().createPerformanceTracker();
            }
     ...
     */
    public static void releaseSceneTracker(MappedScene scene) {
        if (sceneAccessor != null) {
    ...
    //public function getMediaTracker(player : MediaPlayer) : MappedPerformanceTracker {
    //    null;
    //}
    ...
    public static void logEvent(String desc) {
        MappedToolkit.getToolkit().getPerformanceTracker().doLogEvent(desc);
    }
    ...
    public static void outputLog() {
        MappedToolkit.getToolkit().getPerformanceTracker().doOutputLog();
    }
