 ...
 * If a {@code MappedPerspectiveCamera} node is added to the scene graph,
 ...
 * the ParallelCamera.
 * When the MappedScene is resized,
 ...
 * @since JavaFX 2.0
 */
public class MappedPerspectiveCamera extends MappedCamera {
    ...
    static {
        MappedPerspectiveCameraHelper.setPerspectiveCameraAccessor(new MappedPerspectiveCameraHelper.PerspectiveCameraAccessor() {
            @Override
            public MappedNGNode doCreatePeer(MappedNode node) {
                return ((MappedPerspectiveCamera) node).doCreatePeer();
            }

            @Override
            public void doUpdatePeer(MappedNode node) {
                ((MappedPerspectiveCamera) node).doUpdatePeer();
            }
        });
    ...
    public final DoubleProperty fieldOfViewProperty() {
        if (fieldOfView == null) {
            fieldOfView = new SimpleDoubleProperty(MappedPerspectiveCamera.this, "fieldOfView", 30) {
                @Override
                protected void invalidated() {
                    MappedNodeHelper.markDirty(MappedPerspectiveCamera.this, DirtyBits.NODE_CAMERA);
                }
            };
        }
        return fieldOfView;
    ...
    public final BooleanProperty verticalFieldOfViewProperty() {
        if (verticalFieldOfView == null) {
            verticalFieldOfView = new SimpleBooleanProperty(MappedPerspectiveCamera.this, "verticalFieldOfView", true) {
                @Override
                protected void invalidated() {
                    MappedNodeHelper.markDirty(MappedPerspectiveCamera.this, DirtyBits.NODE_CAMERA);
                }
...

    {
        ...
        MappedPerspectiveCameraHelper.initHelper(this);
    }

    /**
     * Creates an empty instance of MappedPerspectiveCamera.
     */
    public MappedPerspectiveCamera() {
        this(false);
    }

    /**
     * Constructs a MappedPerspectiveCamera with the specified fixedEyeAtCameraZero flag.
     *
     ...
     * @since JavaFX 8.0
     */
    public MappedPerspectiveCamera(boolean fixedEyeAtCameraZero) {
        if (!Platform.isSupported(ConditionalFeature.SCENE3D)) {
            String logname = MappedPerspectiveCamera.class.getName();
    ...
    @Override MappedCamera copy() {
        MappedPerspectiveCamera c = new MappedPerspectiveCamera(fixedEyeAtCameraZero);
    ...
    private MappedNGNode doCreatePeer() {
        MappedNGPerspectiveCamera peer = new MappedNGPerspectiveCamera(fixedEyeAtCameraZero);
    ...
    private void doUpdatePeer() {
        MappedNGPerspectiveCamera pgPerspectiveCamera = getPeer();
