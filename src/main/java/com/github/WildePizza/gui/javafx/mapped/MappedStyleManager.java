 ...
 * Stylesheets defined on the MappedScene itself as well as a map of stylesheets for
 * "style"s defined on the MappedNode itself. These containers are kept in the
 * containerMap, key'd by the MappedScene to which they belong. <p> One of the key
 ...
 * takes to match a MappedNode to its eventual StyleHelper, and to reuse the
 ...
 * contains a reference to the id, style class, or classname of the MappedNode, or a
 * compound selector whose "descendant" part is a simple selector which contains
 * a reference to the id, style class, or classname of the MappedNode. <p> During
...
final public class MappedStyleManager {

    /**
     * Global lock object for the MappedStyleManager. MappedStyleManager is a singleton,
     * which loads CSS stylesheets and manages style caches for all scenes.
     * It needs to be thread-safe since a MappedNode or MappedScene can be constructed and
     ...
     * to serialize access to the various state in the MappedStyleManager.
     */
    ...
    private static class InstanceHolder {
        final static MappedStyleManager INSTANCE = new MappedStyleManager();
    }
    /**
     * Return the MappedStyleManager instance.
     */
    public static MappedStyleManager getInstance() {
        return InstanceHolder.INSTANCE;
    }

    private MappedStyleManager() {
    }

    /**
     * A map from a parent to its style cache. The parent is either a MappedScene root, or a
     * MappedParent with author stylesheets. If a MappedScene or MappedParent is removed from the scene,
     * it's cache is annihilated.
     */
    // public for testing
    public static final Map<MappedParent, CacheContainer> cacheContainerMap = new WeakHashMap<>();

    // package for testing
    CacheContainer getCacheContainer(Styleable styleable, MappedSubScene subScene) {

        if (styleable == null && subScene == null) return null;

        MappedParent root = null;

        if (subScene != null) {
            root = subScene.getRoot();

        } else if (styleable instanceof MappedNode) {

            MappedNode node = (MappedNode)styleable;
            MappedScene scene = node.getScene();
            if (scene != null) root = scene.getRoot();

        } else if (styleable instanceof MappedWindow) {
            // this catches the PopupWindow case
            MappedScene scene = ((MappedWindow)styleable).getScene();
            if (scene != null) root = scene.getRoot();
        }
     ...
     */
    public StyleCache getSharedCache(Styleable styleable, MappedSubScene subScene, StyleCache.Key key) {
    ...
    public StyleMap getStyleMap(Styleable styleable, MappedSubScene subScene, int smapId) {
    ...
    /**
     * A list of user-agent stylesheets from MappedScene or MappedSubScene.
     * The order of the entries in this list does not matter since a MappedScene or
     * MappedSubScene will only have zero or one user-agent stylesheets.
     */
    // public for testing
     ...
     *
     * StylesheetContainer<MappedParent> are created and added to stylesheetContainerMap
     * in the method gatherParentStylesheets.
     *
     * StylesheetContainer<MappedScene> are created and added to sceneStylesheetMap in
    ...
    static class StylesheetContainer {
        ...
        final RefList<MappedParent> parentUsers;
        ...
        StylesheetContainer(String fname, Stylesheet stylesheet, byte[] checksum) {
            ...
            this.parentUsers = new RefList<MappedParent>();
    ...
    /**
     * called from MappedWindow when the scene is closed.
     */
    public void forget(final MappedScene scene) {
        ...
        synchronized (styleLock) {
            ...
            while(iter.hasNext()) {
                ...
                Iterator<Reference<MappedParent>> parentIter = container.parentUsers.list.iterator();
                while (parentIter.hasNext()) {

                    Reference<MappedParent> ref = parentIter.next();
                    MappedParent _parent = ref.get();
    ...
    /**
     * called from MappedScene's stylesheets property's onChanged method
     */
    public void stylesheetsChanged(MappedScene scene, Change<String> c) {

        synchronized (styleLock) {
            // Clear the cache so the cache will be rebuilt.
            Set<Entry<MappedParent,CacheContainer>> entrySet = cacheContainerMap.entrySet();
            for(Entry<MappedParent,CacheContainer> entry : entrySet) {
                MappedParent parent = entry.getKey();
    ...
    private void stylesheetRemoved(MappedScene scene, String fname) {
        stylesheetRemoved(scene.getRoot(), fname);
    }

    /**
     * Called from MappedParent's scenesChanged method when the MappedParent's scene is set to null.
     * @param parent The MappedParent being removed from the scene-graph
     */
    public void forget(MappedParent parent) {
        ...
        synchronized (styleLock) {
            ...
            // Do not iterate over children since this method will be called on each from MappedParent#scenesChanged
        }
    }

    /**
     * called from MappedParent's stylesheets property's onChanged method
     */
    public void stylesheetsChanged(MappedParent parent, Change<String> c) {
        synchronized (styleLock) {
            c.reset();
            while(c.next()) {
                if (c.wasRemoved()) {
    ...
    private void stylesheetRemoved(MappedParent parent, String fname) {
     ...
     * called from MappedWindow when the scene is closed.
     */
    public void forget(final MappedSubScene subScene) {

        if (subScene == null) return;
        final MappedParent subSceneRoot = subScene.getRoot();
        ...
        synchronized (styleLock) {
            ...
            //
            // remove any parents belonging to this MappedSubScene from the stylesheetContainerMap
            //
            ...
            while(iter.hasNext()) {
                ...
                while (parentIter.hasNext()) {
                    ...
                    final Reference<MappedParent> ref = parentIter.next();
                    final MappedParent _parent = ref.get();

                    if (_parent != null) {
                        // if this stylesheet refererent is a child of this subscene, nuke it.
                        MappedParent p = _parent;
                        while (p != null) {
    ...
    private void removeStylesheetContainer(StylesheetContainer stylesheetContainer) {
        ...
        synchronized (styleLock) {
            ...
            for(Entry<MappedParent,CacheContainer> entry : cacheContainerMap.entrySet()) {
            ...
            final List<Reference<MappedParent>> parentList = stylesheetContainer.parentUsers.list;

            for (int n=parentList.size()-1; 0<=n; --n) {

                final Reference<MappedParent> ref = parentList.remove(n);
                final MappedParent parent = ref.get();
                ref.clear();
        ...
        try {
            ...
            if (uri.isAbsolute() == false) {
                ...
                if (str.startsWith(skinPrefix) &&
                    ...
                    try {
                        ClassLoader cl = MappedStyleManager.class.getClassLoader();
        ...
        try {
            ...
            try {
                ...
                if ("jar".equals(requestedFileUrI.getScheme())) {
                     ...
                     ** runtime jar
                     */
                    URI styleManagerJarURI = AccessController.doPrivileged((PrivilegedExceptionAction<URI>) () -> MappedStyleManager.class.getProtectionDomain().getCodeSource().getLocation().toURI());
     ...
     */
    // For RT-20643
    public void addUserAgentStylesheet(MappedScene scene, String url) {
     ...
     */
    public void addUserAgentStylesheet(MappedScene scene, Stylesheet ua_stylesheet) {
     ...
     */
    // For RT-20643
    public void setDefaultUserAgentStylesheet(MappedScene scene, String url) {
    ...
    private void userAgentStylesheetsChanged() {
        ...
        List<MappedParent> parents = new ArrayList<>();
        ...
        synchronized (styleLock) {
            ...
            for (MappedParent root : cacheContainerMap.keySet()) {
                if (root == null) {
                    continue;
                }
                parents.add(root);
        ...
        for (MappedParent root : parents) NodeHelper.reapplyCSS(root);
    }

    private List<StylesheetContainer> processStylesheets(List<String> stylesheets, MappedParent parent) {
    ...
    //
    private List<StylesheetContainer> gatherParentStylesheets(final MappedParent parent) {
    ...
    //
    //
    private List<StylesheetContainer> gatherSceneStylesheets(final MappedScene scene) {
    ...
    /**
     * Finds matching styles for this MappedNode.
     */
    public StyleMap findMatchingStyles(MappedNode node, MappedSubScene subScene, Set<PseudoClass>[] triggerStates) {

        final MappedScene scene = node.getScene();
        if (scene == null) {
        ...
        synchronized (styleLock) {
            final MappedParent parent =
                    (node instanceof MappedParent)
                            ? (MappedParent) node : node.getParent();
            ...
            if (cache != null) {
                ...
                if (hasSubSceneUserAgentStylesheet || hasSceneUserAgentStylesheet) {
                    ...
                    // if has both, use MappedSubScene
                    ...
                    if (container.selectorPartitioning != null) {
                        ...
                        final MappedParent root = hasSubSceneUserAgentStylesheet ? subScene.getRoot() : scene.getRoot();
                ...
                if (hasRegionUserAgentStylesheet) {
                    ...
                    if (container.selectorPartitioning != null) {
                        ...
                        container.parentUsers.add((MappedParent)region);
                ...
                // MappedScene stylesheets come next since declarations from
    ...
    ////////////////////////////////////////////////////////////////////////////
    //
    // Classes and routines for mapping styles to a MappedNode
    //
    ////////////////////////////////////////////////////////////////////////////

    private static List<String> cacheMapKey;

    // Each MappedScene has its own cache
    // package for testing
    static class CacheContainer {
        ...
        /**
         * Get the mapping of property to style from MappedNode.style for this node.
         */
        ...
        /**
         * Cache of parsed, inline styles. The key is MappedNode.style.
    ...
    private static class Cache {
        ...
        private StyleMap getStyleMap(CacheContainer cacheContainer, MappedNode node, Set<PseudoClass>[] triggerStates, boolean hasInlineStyle) {
