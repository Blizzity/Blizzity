...
public class MappedNGGroup extends MappedNGNode {
    ...
    private List<MappedNGNode> children = new ArrayList<>(1);
    private List<MappedNGNode> unmod = Collections.unmodifiableList(children);
    private List<MappedNGNode> removed;

    
    private final List<MappedNGNode> viewOrderChildren = new ArrayList<>(1);
    ...
    public List<MappedNGNode> getChildren() { return unmod; }

    
    public void add(int index, MappedNGNode node) {
    ...
    public List<MappedNGNode> getRemovedChildren() {
        return removed;
    }

    public void addToRemoved(MappedNGNode n) {
    ...
    public void remove(MappedNGNode node) {
    ...
    private List<MappedNGNode> getOrderedChildren() {
    ...
    // RenderLock held.
    public void setViewOrderChildren(List<MappedNode> sortedChildren) {
        viewOrderChildren.clear();
        for (MappedNode child : sortedChildren) {
            MappedNGNode childPeer = MappedNodeHelper.getPeer(child);
    ...
    public void renderForcedContent(MappedGraphics gOptional) {
        List<MappedNGNode> orderedChildren = getOrderedChildren();
    ...
    protected void renderContent(MappedGraphics g) {
        ...
        do {
            ...
            Rectangle rclip = MappedPrEffectHelper.getGraphicsClipNoClone(g);
            for (int i = startPos; i < orderedChildren.size(); i++) {
                MappedNGNode child = orderedChildren.get(i);
                ImageData top = MappedNodeEffectInput.
            ...
            if (bot != null && (idValid = bot.validate(fctx))) {
                ...
                MappedPrDrawable botimg = (MappedPrDrawable)bot.getUntransformedImage();
    ...
    @Override
    protected RenderRootResult computeRenderRoot(MappedNodePath path, RectBounds dirtyRegion, int cullingIndex, BaseTransform tx,
                                                 GeneralTransform3D pvTx) {

        // If the MappedNGGroup is completely outside the culling area, then we don't have to traverse down
        // to the children yo.
        if (cullingIndex != -1) {
        ...
        // An MappedNGGroup itself never draws pixels, so we don't have to call super. Just visit
        ...
        for (int resultIdx = orderedChildren.size() - 1; resultIdx >= 0; resultIdx--) {
            ...
            final MappedNGNode child = orderedChildren.get(resultIdx);
    ...
    public void drawDirtyOpts(final BaseTransform tx, final GeneralTransform3D pvTx,
        ...
        for (int childIndex = 0; childIndex < orderedChildren.size(); childIndex++) {
            final MappedNGNode child = orderedChildren.get(childIndex);
