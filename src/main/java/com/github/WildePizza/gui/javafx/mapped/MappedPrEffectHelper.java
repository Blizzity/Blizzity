...
import com.sun.scenario.effect.impl.prism.MappedPrDrawable;
import com.sun.scenario.effect.impl.prism.PrFilterContext;
import com.sun.scenario.effect.impl.prism.MappedPrTexture;

public class MappedPrEffectHelper {

    
    public static void render(Effect effect,
                              MappedGraphics g, float x, float y,
                              Effect defaultInput)
    {
        ...
        if (origtx.is2D()) {
            ...
            MappedNGCamera cam = g.getCameraNoClone();
            BaseTransform inv;
            try {
        ...
        // RT-27555
        if (screen == null) {
            MappedResourceFactory factory = g.getResourceFactory();
        ...
        if (rendertx != null) {
            ...
            // MappedNode rendering so we must disable direct rendering to avoid
            // depth buffer conflicts.
            prinfo = null;
        } else {
            // If none of the above conditions apply, then the MappedPrRenderInfo
            // can represent all information necessary to directly render
            // any ImageData or MappedNode to the destination.
            prinfo = new MappedPrRenderInfo(g);
        }
        boolean valid;
        ...
        do {
            ...
            if (valid = res.validate(fctx)) {
                ...
                MappedTexture tex = ((MappedPrTexture)res.getUntransformedImage()).getTextureObject();
    ...
    static Point2D project(float x, float y, double vw, double vh,
                           MappedNGCamera cam, BaseTransform inv,
                           PickRay tmpray, Vec3d tmpvec, Point2D ret)
    {
        ...
        return tmpray.projectToZeroPlane(inv, cam instanceof MappedNGPerspectiveCamera,
                tmpvec, ret);
    }
    ...
    public static Rectangle getGraphicsClipNoClone(MappedGraphics g) {
        Rectangle rclip = g.getClipRectNoClone();
        if (rclip == null) {
            MappedRenderTarget rt = g.getRenderTarget();
            rclip = new Rectangle(rt.getContentWidth(), rt.getContentHeight());
        }
        return rclip;
    }

    public static void renderImageData(MappedGraphics gdst,
                                       ImageData srcData,
                                       Rectangle dstBounds)
    {
        int w = dstBounds.width;
        int h = dstBounds.height;
        MappedPrDrawable src = (MappedPrDrawable) srcData.getUntransformedImage();
