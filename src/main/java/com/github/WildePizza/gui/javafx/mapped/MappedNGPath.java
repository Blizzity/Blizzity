...
public class MappedNGPath extends MappedNGShape {

    private MappedPath2D p = new MappedPath2D();
    ...
    private int toWindingRule(FillRule rule) {
        if (rule == FillRule.NON_ZERO) {
            return MappedPath2D.WIND_NON_ZERO;
        } else {
            return MappedPath2D.WIND_EVEN_ODD;
        }
    }
    ...
    public MappedPath2D getGeometry() {
        return p;
    }
    ...
    public void updateWithPath2d(MappedPath2D path) {
        p.setTo(path);
        geometryChanged();
    }
