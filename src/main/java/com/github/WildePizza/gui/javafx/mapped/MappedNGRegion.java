...
public class MappedNGRegion extends MappedNGGroup {
    ...
    private MappedShape shape;
    private MappedNGShape ngShape;
    ...
    public void updateShape(Object shape, boolean scaleShape, boolean positionShape, boolean cacheShape) {
        this.ngShape = shape == null ? null : MappedNodeHelper.getPeer(((javafx.scene.shape.Shape)shape));
    ...
    public void updateBorder(Border b) {
        // Make sure that the border instance we store on this MappedNGRegion is never null
    ...
    public void updateBackground(Background b) {
        ...
        // Make sure that the background instance we store on this MappedNGRegion is never null
    ...
    private RegionImageCache getImageCache(final MappedGraphics g) {
        final Screen screen = g.getAssociatedScreen();
        RegionImageCache cache = imageCacheMap.get(screen);
        if (cache != null) {
            MappedRenderTarget tex = cache.getBackingStore();
    ...
    @Override protected RenderRootResult computeRenderRoot(MappedNodePath path, RectBounds dirtyRegion,
    ...
    private void renderAsShape(MappedGraphics g) {
        if (!background.isEmpty()) {
            ...
            MappedRenderTarget cached = null;
            Rectangle rect = null;
            ...
            if (cacheMode != 0 && g.getTransformNoClone().isTranslateOrIdentity() && !(g instanceof PrinterGraphics)) {
                ...
                if (imageCache.isImageCachable(textureWidth, textureHeight)) {
                    ...
                    if (cached != null && render) {
                        MappedGraphics cachedGraphics = cached.createGraphics();
    ...
    private void renderBackgroundShape(MappedGraphics g) {
        if (PULSE_LOGGING_ENABLED) {
            PulseLogger.incrementCounter("MappedNGRegion renderBackgroundShape slow path");
    ...
    private void renderAsRectangle(MappedGraphics g) {
        if (!background.isEmpty()) {
    ...
    private void renderBackgroundRectangle(MappedGraphics g) {
        ...
        if (cache) {
            ...
            if (imageCache.isImageCachable(textureWidth, textureHeight)) {
                ...
                if (cached != null && render) {
                    MappedGraphics cacheGraphics = cached.createGraphics();
        ...
        for (int i = 0, max = images.size(); i < max; i++) {
            ...
            if (imgWidth != 0 && imgHeight != 0) {
                ...
                if (size.isCover()) {
                    ...
                    final MappedTexture texture =
                            g.getResourceFactory().getCachedTexture(prismImage, MappedTexture.WrapMode.CLAMP_TO_EDGE);
    ...
    private void renderBackgroundRectangleFromCache(
            MappedGraphics g, MappedRenderTarget cached, Rectangle rect, int textureWidth, int textureHeight,
    ...
    private void renderBackgroundRectanglesDirectly(MappedGraphics g, float width, float height) {
        final List<BackgroundFill> fills = background.getFills();
        for (int i = 0, max = fills.size(); i < max; i++) {
            final BackgroundFill fill = fills.get(i);
            final Insets insets = fill.getInsets();
            ...
            if (w > 0 && h > 0) {
                ...
                if (radii.isUniform() &&
                    ...
                    if (PULSE_LOGGING_ENABLED) {
                        PulseLogger.incrementCounter("MappedNGRegion renderBackgrounds slow path");
    ...
    private void renderBorderRectangle(MappedGraphics g) {
        ...
        for (int i = 0, max = strokes.size(); i < max; i++) {
            ...
            // The Prism MappedGraphics logic can stroke lines only CENTERED and doesn't know what to do with
    ...
    private BasicStroke createStroke(BorderStrokeStyle sb,
        ...
        if (sb == BorderStrokeStyle.NONE) {
            ...
            // to a MappedGraphics and then use it, that we need to return something
    ...
    private void setBorderStyle(MappedGraphics g, BorderStroke sb, double length, boolean forceCentered) {
    ...
    private void paintTiles(MappedGraphics g, Image img, BorderRepeat repeatX, BorderRepeat repeatY, Side horizontalSide, Side verticalSide,
    ...
    private void paintTiles(MappedGraphics g, Image img, BackgroundRepeat repeatX, BackgroundRepeat repeatY, Side horizontalSide, Side verticalSide,
                            final float regionX, final float regionY, final float regionWidth, final float regionHeight,
                            final int srcX, final int srcY, final int srcW, final int srcH,
                            float tileX, float tileY, float tileWidth, float tileHeight)
    {
        ...
        if (tileX == 0 && tileY == 0 && repeatX == BackgroundRepeat.REPEAT && repeatY == BackgroundRepeat.REPEAT) {
            ...
            // paint loop
            final MappedTexture texture =
                    g.getResourceFactory().getCachedTexture(img, MappedTexture.WrapMode.CLAMP_TO_EDGE);
