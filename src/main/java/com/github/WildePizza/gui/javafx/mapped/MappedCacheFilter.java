...
public class MappedCacheFilter {
    ...
    private MappedRTTexture tempTexture;
    ...
    protected MappedCacheFilter(MappedNGNode node, CacheHint cacheHint) {
        this.node = node;
    ...
    /**
     * Indicates whether this MappedCacheFilter's hint matches the CacheHint
     * passed in.
     */
     ...
     * from MappedGraphics Gems II, as found at
    ...
    void render(MappedGraphics g) {
        ...
        if (needToRenderCache(xform, xformInfo, pixelScaleX, pixelScaleY)) {
            if (PulseLogger.PULSE_LOGGING_ENABLED) {
                PulseLogger.incrementCounter("MappedCacheFilter rebuilding");
            }
            if (cachedImageData != null) {
                Filterable implImage = cachedImageData.getUntransformedImage();
                if (implImage != null) {
        ...
        if (implImage == null) {
            if (PulseLogger.PULSE_LOGGING_ENABLED) {
                PulseLogger.incrementCounter("MappedCacheFilter not used");
            }
    ...
    ImageData createImageData(FilterContext fctx, Rectangle bounds) {
        ...
        try {
            ...
            MappedTexture cachedTex = ((MappedPrDrawable) ret).getTextureObject();
    ...
    void renderNodeToCache(ImageData cacheData,
        ...
        final MappedPrDrawable image = (MappedPrDrawable) cacheData.getUntransformedImage();

        if (image != null) {
            MappedGraphics g = image.createGraphics();
    ...
    void renderNodeToScreen(Object implGraphics) {
        MappedGraphics g = (MappedGraphics)implGraphics;
                             ...
                             double mxt, double myt)
    {
        ...
        MappedTexture cachedTex = ((MappedPrDrawable)implImage).getTextureObject();
        Rectangle cachedBounds = cachedImageData.getUntransformedBounds();
        g.drawTexture(cachedTex, 0, 0,
                cachedBounds.width, cachedBounds.height);
        // FYI: transform state is restored by the MappedNGNode.render() method
    }
    ...
    boolean scrollCacheCapable() {
        ...
        List<MappedNGNode> children = ((MappedNGGroup)node).getChildren();
        if (children.size() != 1) {
            return false;
        }
        MappedNGNode child = children.get(0);
        if (!child.getTransform().is2D()) {
            return false;
        }

        MappedNGNode clip = node.getClipNode();
        if (clip == null || !clip.isRectClip(BaseTransform.IDENTITY_TRANSFORM, false)) {
            return false;
        }

        if (node instanceof MappedNGRegion) {
            MappedNGRegion region = (MappedNGRegion) node;
    ...
    void moveCacheBy(ImageData cachedImageData, double xDelta, double yDelta) {
        MappedPrDrawable drawable = (MappedPrDrawable) cachedImageData.getUntransformedImage();
        ...
        final MappedGraphics g = drawable.createGraphics();
        if (tempTexture != null) {
            tempTexture.lock();
        ...
        if (tempTexture == null) {
            tempTexture = g.getResourceFactory().
                    createRTTexture(drawable.getPhysicalWidth(), drawable.getPhysicalHeight(),
                            MappedTexture.WrapMode.CLAMP_NOT_NEEDED);
        }
        final MappedGraphics tempG = tempTexture.createGraphics();
        tempG.clear();
